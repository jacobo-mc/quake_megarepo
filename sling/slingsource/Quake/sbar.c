...
{
	return; //again, Sling doesn't need a status bar, so remove any pics called for it
...
void Sbar_Draw (void)
{
	return; //Sling doesnt need a status bar, therefore don't draw it at all
...
{
	return; // maybe needs later changing to show time, and not secrets and kills. May substitute this with just printing accurate time in qc after finish level.
	qpic_t	*pic;
	int	dig;
	int	num;
