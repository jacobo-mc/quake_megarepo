...
cvar_t	cl_rollangle = {"cl_rollangle", "0", CVAR_ARCHIVE};

cvar_t	cl_bob = {"cl_bob","0", CVAR_ARCHIVE};
...
cvar_t	v_nodrift = {"v_nodrift", "1", CVAR_ARCHIVE}; // allow player to enable or disable drifting
...
void V_CalcRefdef (void)
{
	...
	//if (v_nodrift.value == 0) //nodrift only in players view so centerview still works
	//{
	//	V_DriftPitch ();
	//}
...
void V_Init (void)
{
	...
	Cvar_RegisterVariable (&v_nodrift); //add v_nodrift
}
