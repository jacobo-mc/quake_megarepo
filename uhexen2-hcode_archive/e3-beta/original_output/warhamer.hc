...
local float strikelength = 0.00000;
   ...
   numstrikes = ((random () * 5.00000) + 1.00000);
   while ( (numstrikes > 0.00000) ) {

      zapangle_x = ((random () * 720.00000) - 360.00000);
      zapangle_y = ((random () * 720.00000) - 360.00000);
      zapangle_z = ((random () * 720.00000) - 360.00000);
      makevectors (zapangle);
      fromspot = (self.origin + (v_forward * 16.00000));
      tospot = (self.origin + (v_forward * ((random () * strikelength) + 32.00000)));
      ...
      WriteCoord (MSG_BROADCAST, fromspot);
      WriteCoord (MSG_BROADCAST, fromspot_y);
      WriteCoord (MSG_BROADCAST, fromspot_z);
      WriteCoord (MSG_BROADCAST, tospot);
      WriteCoord (MSG_BROADCAST, tospot_y);
      WriteCoord (MSG_BROADCAST, tospot_z);
      LightningDamage (fromspot, tospot, self.controller, 3.00000, "lightning");
      self.level += 1.00000;
      numstrikes -= 1.00000;
...
local vector dir = '0.00000 0.00000 0.00000';
   ...
   if ( ((self.lifetime < time) || ((distance < 16.00000) && (self.bloodloss < time))) ) {
   ...
   if ( (self.controller.health <= 0.00000) ) {
...
local entity missile;
   ...
   newmis = spawn ();
   missile.movechain = newmis;
   newmis.solid = SOLID_NOT;
   newmis.movetype = MOVETYPE_FLYMISSILE;
   setmodel (newmis, "models/glowball.mdl");
   setorigin (newmis, missile.origin);
   newmis.owner = missile;
   if ( (random () < 0.50000) ) {
...
void  (vector org,float markerskin)spawn_marker =  {
   newmis = spawn ();
   newmis.skin = markerskin;
...
local float hit_count = 0.00000;
   ...
   if ( (trace_fraction < 1.00000) ) {
      ...
      spawn_marker (trace_endpos, 1.00000);
