...
local float len2 = 0.00000;
   ...
   if ( (self.finaldest == self.origin) ) {
      ...
      if ( (self.state == STATE_DOWN) ) {
         ...
         self.think = door_hit_bottom_final;
      } else {
...
void  ()door_bounce_next =  {
   self.velocity = (self.velocity * 0.50000);
   self.velocity = '0.00000 0.00000 0.00000';
   if ( '0.00000 0.00000 0.00000' ) {
...
local float len = 0.00000;
   ...
   self.velocity = (self.velocity * 2.00000);
...
void  ()door_blocked =  {
   ...
   if ( (((other.flags & FL_ONGROUND) && (self.movedir_x || self.movedir_z)) && (other.origin_z > self.origin_z)) ) {
...
local entity starte;
   ...
   self.no_puzzle_msg = string_null;
...
local vector t2 = '0.00000 0.00000 0.00000';
   ...
   if ( (door.classname == "door") ) {
      ...
      if ( (door.classname == "door_rotating") ) {
...
local vector newvect = '0.00000 0.00000 0.00000';
   ...
   SUB_CalcAngleMove ((self.angles + newvect), self.speed, SUB_Null);
};
...
local vector vec = '0.00000 0.00000 0.00000';
   ...
   if ( (self.spawnflags & 64.00000) ) {
      ...
      self.spawnflags -= 64.00000;

   }
   if ( (self.spawnflags & 128.00000) ) {

      self.spawnflags -= 128.00000;

   }
   if ( (self.spawnflags & 8.00000) ) {

      self.spawnflags = (self.spawnflags & 64.00000);

   }
   if ( (self.spawnflags & 16.00000) ) {

      self.spawnflags = (self.spawnflags & 128.00000);

   }
   if ( (self.spawnflags & 32.00000) ) {

      self.spawnflags = (self.spawnflags | DOOR_TOGGLE);
