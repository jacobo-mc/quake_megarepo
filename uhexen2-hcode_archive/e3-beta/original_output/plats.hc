...
void  ()train_wait =  {
   ...
   if ( ((self.flags == 0.00000) || (self.flags == 2.00000)) ) {
...
local float targ_aspeed = 0.00000;
   ...
   if ( (!self.flags && (self.spawnflags & TRAIN_RETURN)) ) {
...
void  ()misc_teleporttrain =  {
   ...
   self.blocked = train_blocked;
   self.use = train_use;
   self.avelocity = '100.00000 200.00000 300.00000';
   self.noise = "misc/null.wav";
   precache_sound ("misc/null.wav");
   self.noise1 = "misc/null.wav";
   precache_sound ("misc/null.wav");
...
void  ()waterplat_think =  {
   tracearea (self.origin, self.origin, self.mins, self.maxs, FALSE, self);
   if ( ((trace_fraction < 1.00000) && (trace_ent.classname == "player")) ) {

      centerprint (trace_ent, "inwater!\n");
      if ( !(trace_ent.flags2 & FL_FAKE_WATER) ) {

         trace_ent.flags2 += FL_FAKE_WATER;

      }

   }
   self.think = waterplat_think;
...
void  ()crusher_hit_bottom =  {
   sound (self, CHAN_VOICE, self.noise1, 1.00000, ATTN_NORM);
   self.state = STATE_BOTTOM;
   if ( (!self.active && (self.spawnflags & CRUSH_ENDPOS)) ) {
