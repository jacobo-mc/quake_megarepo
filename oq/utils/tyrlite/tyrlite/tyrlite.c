...
LightWorld(void)
{
	...
	lit_filebase = file_end = filebase + (MAX_MAP_LIGHTING / 4);

	// Need to align the litfile to a 12 byte boundary
	if ((unsigned long)lit_filebase % 12)
	    lit_filebase += 12 - ((unsigned long)lit_filebase % 12);
	lit_file_p = lit_filebase;
	lit_file_end = filebase + MAX_MAP_LIGHTING;
    } else
