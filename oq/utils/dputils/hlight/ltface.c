...
//	if (dist > light->light)
	// LordHavoc: had to make this bigger because of the change in falloff
	// (note the 64.0f is 8.0f squared, which is more than the needed 2.0f
	// but is there to try to limit the light distance)
	if ((light->light * light->light * (1.0f/64.0f)) < (dist * dist * falloff2 + 1.0f))
	{
		c_culldistplane++;
		return;
	}
...
//
	...
	for (c=0 ; c<l->numsurfpt*3 ; c+=3, surf+=3) // LordHavoc: color
	{
		...
		add = light->light * 0.5f / (dist * dist * falloff2 + 1.0f);
		add *= angle;
		if (add <= 0)
			continue;
		...
		if ((lightsamp[c]+lightsamp[c+1]+lightsamp[c+2]) >= 3)		// ignore real tiny lights
			hit = true;
	}
		...
		for (t2 = 0, t=0 ; t<=l.texsize[1] ; t2 += w3, t++)
		{
			for (s2 = t2, s=0 ; s<=l.texsize[0] ; s++, s2 += 6, c+=3)
			{
				...
				{	// filtered sample
					...
					//green = (light[t*2*w+s*2)*3+1] + light[(t*2*w+s*2+1)*3+1] + light[((t*2+1)*w+s*2)*3+1] + light[((t*2+1)*w+s*2+1)*3+1]) * 0.25;
				}
				else
				{
					red = light[c];
