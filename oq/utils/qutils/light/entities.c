...
// go through all the entities
	...
	// go through all the keys in this entity
		...
		if (!strncmp (entity->classname, "light", 5))
		{
			...
			if (!entity->light)       entity->light = DEFAULTLIGHTLEVEL;
			if (!entity->noise)       entity->noise = noise;
			if (!entity->persistence) entity->persistence = 1;

			// emulate IKLITE attenuation based on angle key
			if (iklite)
			{
				if (entity->angle == 1) entity->attenuation = LIGHT_REALISTIC;
				if (entity->angle == 2) entity->attenuation = LIGHT_NO_ATTEN;
				if (entity->angle == 3) entity->attenuation = LIGHT_INVERSE;
			}

			// SerpLord - calculate radius for different attenuation types

			// allow negative light intensities
			...
			switch (entity->attenuation)
			{
				case LIGHT_RADIUS:
					...
					if (!entity->radius) entity->radius = intensity;
					break;
				...
				case LIGHT_REALISTIC:
					if (cutoff)
					{
						...
						cutoff_range = sqrt (intensity / cutoff);
