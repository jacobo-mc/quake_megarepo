...
void CopyQFiles (int blocknum)
{
	...
	sprintf (destfile, "%spak%i.pak", qdir, blocknum);
	...
	for (i=0 ; i<nummodels ; i++)
	{
		if (precache_models_block[i] != blocknum)
			continue;
		sprintf (srcfile,"%s%s", qdir, precache_models[i]);
	...
	for (i=0 ; i<numfiles ; i++)
	{
		if (precache_files_block[i] != blocknum)
			continue;
		sprintf (srcfile,"%s%s", qdir, precache_files[i]);
