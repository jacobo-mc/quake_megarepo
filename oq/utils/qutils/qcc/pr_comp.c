...
#ifdef HASH_DEFS
static int Hash(int s_name,def_t *scope)
{
	return (s_name^((int)scope))&(HASH_SIZE-1);
}
...
============
*/
static string_t name_immediate;
...
def_t	*PR_ParseImmediate (void)
{
	...
	if (!name_immediate)
		name_immediate=CopyString("IMMEDIATE");

	if (pr_immediate_type == &type_string)
		pr_immediate.string = CopyString (pr_immediate_string);

	hash=pr_immediate._int&(HASH_SIZE-1);
// check for a constant with the same value
	for (cn=pr.search[hash];cn;cn=cn->search_next)
	{
		if (!cn->initialized)
			continue;
...
// allocate a new one
	...
	cn->search_next = pr.search[hash];
	pr.search[hash] = cn;

	cn->type = pr_immediate_type;
	cn->name = "IMMEDIATE";
	cn->s_name = name_immediate;
	cn->initialized = 1;
...
def_t *PR_GetDef (type_t *type, char *name, def_t *scope, qboolean allocate)
{
	...
	s_name=CopyString(name);
	hash=Hash(s_name,NULL);
// see if the name is already in use
	old = &pr.search[hash];
	for (def = *old ; def ; old=&def->search_next,def = *old)
//		if (!strcmp(def->name,name) )
		if (def->s_name==s_name)
		{
			...
			*old = def->search_next;
			def->search_next = pr.search[hash];
			pr.search[hash] = def;
			return def;
		}
...
def_t *PR_Statement ( opcode_t *op, def_t *var_a, def_t *var_b)
{
	...
	statement_linenums[statement-statements] = pr_current_source->line;
...
============
*/
qboolean	PR_CompileFile (char *string)
{	
	if (!pr.memory)
		Error ("PR_CompileFile: Didn't clear");

	pr_file_start = string;
