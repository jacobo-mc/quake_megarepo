...
pr_source_t *pr_source=NULL;
int	pr_num_source=0;
static pr_source_t src_unknown={"<unknown>",0};
pr_source_t *pr_current_source=&src_unknown;
...
void PR_PrintNextLine (void)
{
	...
	printf ("%3i:",pr_current_source->line);
...
==============
*/
static void PR_NewLine (int comment)
{
	pr_file_p++;
	pr_current_source->line++;
	...
	{
		int line;
		char buf[32],file[512],*b;
		int type;

		pr_file_p++;
		pr_file_p++;
		for (b=buf;*pr_file_p!=' ';) *b++=*pr_file_p++;
		*b=0;
		line=atoi(buf);
		
		pr_file_p++;
		pr_file_p++;
		for (b=file;*pr_file_p!='"';) *b++=*pr_file_p++;
		*b=0;
		...
		if (type==2)
		{
			...
			{
				pr_num_source--;
				pr_current_source=&pr_source[pr_num_source-1];
				pr_current_source->line=line;
			}
			PR_ClearGrabMacros();
			s_file=CopyString(pr_current_source->name);
		}
		else
		if (type==1 || pr_num_source==0)
		{
			pr_source=Q_realloc(pr_source,sizeof(pr_source_t)*(pr_num_source+1));
			pr_current_source=&pr_source[pr_num_source++];
			memset(pr_current_source,0,sizeof(pr_source_t));
			strcpy(pr_current_source->name,file);
			pr_current_source->line=line;
			s_file=CopyString(pr_current_source->name);
		}
		else
...
void PR_LexStart(void)
{
	pr_num_source=0;
	pr_current_source=&src_unknown;
	pr_current_source->line=0;
	PR_ClearGrabMacros ();	// clear the frame macros
	s_file=0;
	pr_file_p = pr_file_start-1;
	PR_NewLine(0);
}
...
void PR_ParseError (char *error, ...)
{
	...
	for (i=pr_num_source-1;i>=0;i--)
		fprintf (stderr, "%s:%i\n", pr_source[i].name, pr_source[i].line);
